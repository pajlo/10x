...
const handleSave = async () => {
  ...
  if (editingFlashcard.value) {
    result = await flashcardsStore.updateFlashcard(editingFlashcard.value.id, front.value, back.value);
  } else {
...
<template>
  <div class="my-flashcards">
      ...
      >
        ...
        <div class="px-5 py-3 bg-gray-50 flex justify-end gap-2">
          <BaseButton 
            variant="secondary"
            size="sm"
            @click="showEditDialog(flashcard)"
          >
            Edytuj
          </BaseButton>
          <BaseButton 
    ...
    >
      ...
      <div class="space-y-4">
        <div>
          <label for="front" class="block text-sm font-medium text-gray-700">Przód fiszki (pytanie)</label>
          <textarea
            id="front"
            v-model="front"
            rows="3"
        ...
        <div>
          <label for="back" class="block text-sm font-medium text-gray-700">Tył fiszki (odpowiedź)</label>
          <textarea
            id="back"
            v-model="back"
            rows="3"
